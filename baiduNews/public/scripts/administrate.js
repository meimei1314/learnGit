	...
	$.ajax({
		...
		success:function(data) {
			data.forEach(function(value,index) {
				...
				var td_time = $("<td></td>").text(value.news_time.split("T")[0]).appendTo(tr);
...
function creatNews() {
	...
	$("#btn_submit").on("click",function(event) {
		...
		if( $("#news_title").val()==="" || $("#news_time").val()==="" 
			...
			if(  $("#news_title").parent().hasClass("has-error") ) {
				$("#news_title").parent().removeClass("has-error");
			}

			if(  $("#news_time").parent().hasClass("has-error") ) {
				$("#news_time").parent().removeClass("has-error");
			}

			if(  $("#news_src").parent().hasClass("has-error") ) {
	...
	$(".table tbody").on("click",".btn-primary",function() {
		...
		$.ajax({
			...
			success:function(data) {
				...
				$("#Unews_time").val(data[0].news_time.split("T")[0]);
